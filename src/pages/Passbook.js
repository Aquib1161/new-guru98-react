...
class Passbook extends React.Component {
  ...
  getPassbook = async () => {
    ...
    try {
      const response = await httpPost("Passbook", {});
  ...
  render() {
    const { passbook, is_fetching } = this.state;
    const { t } = this.props;
    const team_data = get(passbook, "team_data", []);

    const isOld = localStorage.getItem("is_old_theme") === "true";

    if (isOld) return <OldPassbook passbook={passbook} is_fetching={is_fetching} />;
...
export default withTranslation("rules")(Passbook);
